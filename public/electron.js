...
mainPageRoutes.get('/creditors/entities/:id', (req, res) => {
    pool.request().query(`SELECT RTRIM(BSSI_FACILITY_ID) AS ENTITY FROM B3900220 WITH(NOLOCK) WHERE VENDORID = '${req.params.id}'`)
        .then(result => {
            //console.log(result);
            res.json(result);
        });
...
mainPageRoutes.get('/creditors/:id', (req, res) => {
    pool.request().query(`SELECT RTRIM(VENDORID) AS VENDORID, RTRIM(VENDNAME) AS VENDNAME FROM PM00200 WITH(NOLOCK) WHERE VENDORID LIKE '${req.params.id}%'`)
        .then(result => {
            //console.log(result);
            res.json(result);
        });
...
mainPageRoutes.get('/debtors/entities/:id', (req, res) => {
    pool.request().query(`SELECT RTRIM(BSSI_FACILITY_ID) AS ENTITY FROM B3900270 WITH(NOLOCK) WHERE CUSTNMBR = '${req.params.id}'`)
        .then(result => {
            //console.log(result);
            res.json(result);
        });
...
mainPageRoutes.get('/debtors/:id', (req, res) => {
    pool.request().query(`SELECT RTRIM(CUSTNMBR) as CUSTNMBR, RTRIM(CUSTNAME) as CUSTNAME FROM RM00101 WITH(NOLOCK) WHERE CUSTNMBR LIKE '${req.params.id}%'`)
        .then(result => {
            console.log(result);
            res.json(result);
        });
